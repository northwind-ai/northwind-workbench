...
describe('scanWorkspace', () => {
  ...
  it('discovers Nx projects from apps/ and libs/', async () => {
    ...
    expect(packages.find((p) => p.name === '@fixture/web')!.runtime).toBe('browser');
  });
...
describe('createRunner (integration)', () => {
  it('scans + runs all checks against a real fixture workspace', async () => {
    const runner = createRunner({ cwd: fixture('pnpm-ws'), clock: () => '2020-01-01T00:00:00.000Z' });
    ...
    for (const report of run.reports) {
      ...
      expect(report.checks.find((c) => c.checkId === 'package_json_valid')!.status).toBe('pass');
    }

...
describe("computeScore", () => {
  it("returns 100 when everything passes", () => {
    expect(computeScore([check("pass"), check("pass"), check("pass")])).toBe(
      100,
    );
  });
  ...
  it("subtracts moderately for a warning", () => {
    expect(
      computeScore([check("warn", "medium"), check("pass"), check("pass")]),
    ).toBe(92);
  });
  ...
  it("does not penalize skip or unknown", () => {
    expect(computeScore([check("skip"), check("unknown"), check("pass")])).toBe(
      100,
    );
  });
...
describe("computeStatus", () => {
  ...
  it("warns if any check warns and none fail", () => {
    expect(computeStatus([check("pass"), check("warn", "medium")])).toBe(
      "warn",
    );
  });
  it("passes when only pass/skip/unknown", () => {
    expect(
      computeStatus([check("pass"), check("skip"), check("unknown")]),
    ).toBe("pass");
  });
});
...
describe("buildReport", () => {
  it("assembles score, confidence and status together", () => {
    const report = buildReport(
      fakePkg("p"),
      [check("pass"), check("fail", "critical")],
...
describe("summarize", () => {
  const mk = (
    ...
    confidence: PackageHealthReport["confidence"],
  ): PackageHealthReport => ({
    package: fakePkg(id),
    checks: [],
    score,

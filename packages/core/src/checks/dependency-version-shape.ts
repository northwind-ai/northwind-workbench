...
export const dependencyVersionShape = defineCheck({
  ...
  description: 'Every dependency/devDependency/peerDependency version specifier has a recognizable shape.',
  severity: 'low',
  weight: 1,
  ...
  async run({ package: pkg }) {
    ...
    for (const name of names) {
      const spec = all[name];
      if (typeof spec !== 'string' || spec.trim() === '' || !VALID.test(spec.trim())) {
        bad.push(`${name}: ${JSON.stringify(spec)}`);
      }
    }

    if (bad.length === 0) return pass(`${names.length} version specifier(s) look valid`);
    return warn('low', `${bad.length} suspicious version specifier(s)`, { evidence: bad });
  },
});

...
describe("package_json_valid", () => {
  ...
  it("fails when the manifest is invalid", async () => {
    const out = await packageJsonValid.run(
      makeCtx({}, { manifestValid: false, warnings: ["Invalid JSON"] }),
    );
...
describe("package_name_present", () => {
  it("passes when a name is set", async () => {
    expect(
      (await packageNamePresent.run(makeCtx({ name: "pkg" }))).status,
    ).toBe("pass");
  });
  it("fails when name is blank", async () => {
    expect((await packageNamePresent.run(makeCtx({ name: "" }))).status).toBe(
      "fail",
    );
  });
});
...
describe("entrypoint_exists", () => {
  it("passes when a declared entry resolves", async () => {
    const out = await entrypointExists.run(
      makeCtx({ main: "index.js" }, { files: [join(ROOT, "index.js")] }),
    );
    expect(out.status).toBe("pass");
  });
  it("fails when declared entries are all missing", async () => {
    expect(
      (await entrypointExists.run(makeCtx({ main: "dist/index.js" }))).status,
    ).toBe("fail");
  });
  it("passes via an index fallback when nothing is declared", async () => {
    const out = await entrypointExists.run(
      makeCtx({}, { files: [join(ROOT, "index.js")] }),
    );
    expect(out.status).toBe("pass");
  });
...
describe("main_module_exists", () => {
  ...
  it("fails when main is declared but missing", async () => {
    expect(
      (await mainModuleExists.run(makeCtx({ main: "dist/index.js" }))).status,
    ).toBe("fail");
  });
});
...
describe("types_entry_exists", () => {
  ...
  it("fails when the declared types file is missing", async () => {
    expect(
      (await typesEntryExists.run(makeCtx({ types: "dist/index.d.ts" })))
        .status,
    ).toBe("fail");
  });
});
  ...
  it("passes when the peer is present in the workspace root", async () => {
    const out = await missingPeerDependencies.run(
      makeCtx(
        ...
        {
          files: [
            join(workspace.root, "node_modules", "react", "package.json"),
          ],
        },
      ),
    );
...
describe("required_scripts_present", () => {
  ...
  it("passes when common scripts exist", async () => {
    const out = await requiredScriptsPresent.run(
      makeCtx({
        main: "index.js",
        scripts: { build: "x", test: "y", typecheck: "z", lint: "w" },
      }),
    );
    expect(out.status).toBe("pass");
  });
...
describe("dependency_version_shape", () => {
  ...
  it("warns for malformed specifiers", async () => {
    const out = await dependencyVersionShape.run(
      makeCtx({ dependencies: { a: "^^1.0.0", b: "" } }),
    );
    expect(out.status).toBe("warn");
...
describe("module_resolution_check", () => {
  it("skips when nothing is declared", async () => {
    expect((await moduleResolutionCheck.run(makeCtx({}))).status).toBe("skip");
  });
  it("fails when a declared main is missing", async () => {
    expect(
      (await moduleResolutionCheck.run(makeCtx({ main: "dist/index.js" })))
        .status,
    ).toBe("fail");
  });
});

describe("exports_map_check", () => {
  it("skips when there is no exports field", async () => {
    expect((await exportsMapCheck.run(makeCtx({}))).status).toBe("skip");
  });
  it("fails when exports mixes subpaths and conditions", async () => {
    const out = await exportsMapCheck.run(
      makeCtx({ exports: { ".": "./index.js", import: "./index.js" } }),
    );
    expect(out.status).toBe("fail");
  });
});

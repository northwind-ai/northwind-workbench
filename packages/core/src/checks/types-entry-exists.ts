...
export const typesEntryExists = defineCheck({
  ...
  description: 'The declared "types"/"typings" entry resolves to a real .d.ts file.',
  severity: 'medium',
  weight: 1,

  async run({ package: pkg, host }) {
    const rel = pkg.manifest.types ?? pkg.manifest.typings;
    if (typeof rel !== 'string') return skip('No "types"/"typings" field declared');

    if (await host.fileExists(join(pkg.root, rel))) return pass(`Types resolve: ${rel}`);

...
import { defineCheck, fail, pass, warn, type PackageManifest } from '@package-workbench/plugin-sdk';
...
const INDEX_FALLBACKS = ['index.js', 'index.mjs', 'index.cjs', 'src/index.ts', 'index.ts'];

/** Declared entry points, ignoring glob targets we can't statically resolve. */
function declaredEntries(m: PackageManifest): string[] {
  const out: string[] = [];
  for (const f of [m.main, m.module, m.types, m.typings]) if (typeof f === 'string') out.push(f);
  collectStrings(m.exports, out);
  if (typeof m.bin === 'string') out.push(m.bin);
  else if (m.bin && typeof m.bin === 'object') out.push(...Object.values(m.bin));
...
export const entrypointExists = defineCheck({
  ...
  description: 'At least one declared entry point (or an index fallback) exists on disk.',
  severity: 'high',
  weight: 2,
  ...
  async run({ package: pkg, host }) {
    ...
    if (declared.length === 0) {
      for (const fallback of INDEX_FALLBACKS) {
        if (await host.fileExists(join(pkg.root, fallback))) return pass(`Resolved via ${fallback}`);
      }
    ...
    if (present.length > 0) return pass(`${present.length}/${declared.length} declared entry point(s) resolve`);

...
export const missingPeerDependencies = defineCheck({
  ...
  description: 'Every required peer dependency is installed in the package or workspace root.',
  severity: 'high',
  weight: 1,
  ...
  async run({ package: pkg, workspace, host }) {
    ...
    for (const name of names) {
      ...
      const inPkg = await host.fileExists(join(pkg.root, 'node_modules', name, 'package.json'));
      const inRoot = await host.fileExists(join(workspace.root, 'node_modules', name, 'package.json'));

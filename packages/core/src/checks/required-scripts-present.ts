...
export const requiredScriptsPresent = defineCheck({
  ...
  description:
    "Reports which of build/test/typecheck/lint scripts are defined (does not run them).",
  severity: "low",
  weight: 1,

  async run({ package: pkg }): Promise<HealthCheckOutcome> {
    const scripts = pkg.scripts;
    const present = COMMON.filter(
    ...
    const missingImportant =
      wantsBuildTest &&
      (!present.includes("build") || !present.includes("test"));

    if (missingImportant) {
      return warn("low", "Library is missing build and/or test script", {
        details: detail,
      });
    }
    if (present.length === 0) {
    ...
    return pass(`${present.length}/${COMMON.length} common scripts present`, {
      details: detail,
    });
  },
});
